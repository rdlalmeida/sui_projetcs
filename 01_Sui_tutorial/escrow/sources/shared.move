...
/// An object held in escrow. The escrowed object is added as a Dynamic Object Field so it can still be looked-up
...
public fun swap<T: key + store, U: key + store>(
    ...
    let escrowed: T = dof::remove<EscrowedObjectKey, T>(&mut escrow.id, EscrowedObjectKey {});
    ...
    assert!(exchange_key == object::id(&key), EMismatchedExchangedObject);
...
public struct EscrowCreated has copy, drop {
    ...
    escrow_id: ID,
    /// The ID od the `Key` that unlocks the requested object.
    key_id: ID,
...
fun test_successful_swap() {
    ...
    // Alice creates a public Escrow holding the object they are willing to share, and the object they want from Bob.
    let i1 = {
        ts.next_tx(ALICE);
        let c = test_coin(&mut ts);
        let cid = object::id(&c);
    ...
    // Bob responds by offering their object, and gets Alice's object in return
    // docs::#bob
    {
        ts.next_tx(BOB);
...
fun test_mismatch_object() {
    let mut ts: Scenario = ts::begin(@0x0);
    ...
    // Alice wants to trade with Bob, but Alice has asked for an object (via its `exchange_key`) that Bob has not put up for the swap.
    {
        ts.next_tx(ALICE);
        let c = test_coin(&mut ts);
        let cid = object::id(&c);
    ...
    // When Bob tries to complete the swap, it will fail, because they cannot meet Alice's requirements.
    {
        ts.next_tx(BOB);
        let escrow: Escrow<Coin<SUI>> = ts.take_shared();
        let k2: Key = ts.take_from_sender();
...
fun test_object_tamper() {
    ...
    // Bob has a change of heart, so they unlock the object and tamper with it before initiating the swap, but it won't be possible for Bob to hid their tampering.
    {
        ts.next_tx(BOB);
...
fun test_return_to_sender() {
    ...
    // Alice puts the object they want to trade
    let cid = {
        ts.next_tx(ALICE);
        let c = test_coin(&mut ts);
        let cid = object::id(&c);
...
fun test_return_to_sender_failed_swap() {
    ...
    // Alice creates a public Escrow hold the object they are willing to share, and the object they want from Bob.
    {
        ts.next_tx(ALICE);
        let c = test_coin(&mut ts);
        create(c, ik2, BOB, ts.ctx());
