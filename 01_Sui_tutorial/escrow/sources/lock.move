/**
    ...
    The `lock` module offers an API for wrapping any object that has `store` and protecting it with a single use `Key`.

    This is used to commit to swapping a particular object in a particular, fixed state during escrow.
**/
module escrow::lock;
...
/// The `name` of the DOF that holds the Locked object. Allows better discoverability for the locked object.
public struct LockedObjectKey has copy, drop, store {}

/// A wrapper that protects access to `obj` by requiring access to a `Key`. Used to ensure an object is not modified if it might be involved in a swap. Object is added as a Dynamic Object field so that it can still be looked-up.
...
/// Unlock the object in `locked`, consuming the `key`. Fails if the wrong `key` is passed in for the locked object.
...
fun test_lock_unlock() {
    let mut ts: Scenario = ts::begin(@0xA);
...
fun test_lock_key_mismatch() {
    ...
    let (l1, _k1) = lock(coin, ts.ctx());
    let (_l2, k2) = lock(another_coin, ts.ctx());

    let _key = l1.unlock(k2);
    abort 1337
}
